...
def process_db_request(db_filepath, handler, args):
...
class Registry:
    ...
    def add_q_model(self, id_, date_time_created, description):
        return process_db_request(self.db_filepath, add_q_model_handler, [id_, date_time_created, description])

    def add_race(self, game_info):
        return process_db_request(self.db_filepath, add_race_data_handler, [game_info])

    def get_latest_q_model_object(self):
        return process_db_request(self.db_filepath, get_lastest_q_model_object_handler, [])

    def get_race_data_objects(self, ultimate_performance_mode_entries=False):
        return process_db_request(self.db_filepath, get_race_objects_handler, [ultimate_performance_mode_entries])

    def get_race_data_objects_in_replay_buffer(self):
        return process_db_request(self.db_filepath, get_race_objects_in_replay_buffer_handler, [])

    def get_court_settings_object(self, game_info):
        return process_db_request(self.db_filepath, get_court_setting_handler, [game_info])
